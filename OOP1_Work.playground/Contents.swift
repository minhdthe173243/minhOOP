...
// dùng 2 tinh chat trong OOP: inheritance(kế thừa) , polymorphism(đa hình), encapsulation(đóng gói)
class Officers {
...
class ManagerOfficers {
    ...
    enum TypeOfficers {
        case Worker, Engnieer , Employee, All
    }
    
    enum Sort {
        case assending, decrease
    }
    
    
    func sortBySalaryOfTypeOfficers(obj: TypeOfficers, type: Sort){
        switch obj {
        case .Worker:
...
print("\n")
print("---listSort---")
manager.sortBySalaryOfTypeOfficers(obj: .All, type: .decrease)
